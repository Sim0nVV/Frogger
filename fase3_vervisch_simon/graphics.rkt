...
; Zet deze boolean op #t om ervoor te zorgen dat een toets die ingedrukt blijft niet continu
; `'down <key>` stuurt naar de key callback handler.
    ...
    (define my-canvas%
      (class canvas% ; The base class is canvas%
        ...
        (define/override (on-char event)
          ...
          (if (eq? evt 'release)
              (handle-keyboard 'up (send event get-key-release-code))
              (handle-keyboard 'down evt)))
